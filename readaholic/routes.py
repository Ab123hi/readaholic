...
def books():
    ...
    if form.validate_on_submit():
        # filename = save_cover_image(form.upload_cover_image)
        ...
        _cover_image_file = save_cover_image(form.upload_cover_image)
        _tiny_summary = form.data['tiny_summary']
        book = Book(title=_title, author=_author, isbn=_isbn, genre=_genre, shop_link=_shop_link, rating=_rating, tiny_summary=_tiny_summary, image=_cover_image_file)
        try:
            db.session.add(book)
            db.session.commit()
